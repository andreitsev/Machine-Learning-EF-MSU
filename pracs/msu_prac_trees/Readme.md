...
если нет, то вот так:
```angular2html
docker run --rm -it -v $PWD:/tmp mlefmsu/prac_trees /bin/bash -c 'cp -r /tmp/* /prac_folder; cd /prac_folder; python3 unzip_private_tests.py; python3 test_submission.py; if [[ -f main_task_score.txt ]]; then mv main_task_score.txt ../tmp/; fi'                                                      
```
